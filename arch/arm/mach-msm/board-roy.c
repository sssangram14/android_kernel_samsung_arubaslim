...
static struct msm_gpio qup_i2c_gpios_io[] = {
	...
	{ GPIO_CFG(61, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
		"qup_sda" },
#if !defined(CONFIG_TOUCHSCREEN_MELFAS_KYLE) && !defined(CONFIG_MACH_ROY_DTV)
...
static void __init msm7x27a_init_ebi2(void)
{
	...
	//	writel(ebi2_cfg, ebi2_cfg_ptr);
		...
		ebi2_cfg = readl(ebi2_cfg_ptr);
		ebi2_cfg &= ~(0x3 << 4);  // clean the priority of XMEMC
		ebi2_cfg &= ~(0x3 << 0);  // clean the priority of LCDC
	...
	ebi2_cfg |= (0x5 << 4); // max WRITE_WAIT  org 7
	ebi2_cfg |= (0xF << 16); // max initial latency WR org 2F
	ebi2_cfg |= (0x6 << 24); // max hold write  org A
	
	ebi2_cfg |= (0x5 << 0); // max read wait  org 7
	ebi2_cfg |= (0xF << 8); // max initial latency RD  org 1F
...
//	ebi2_cfg |= (0x1 << 31); // A/D Muxed
	...
	ebi2_cfg |= (0x6 << 24); // max hold read  org A
