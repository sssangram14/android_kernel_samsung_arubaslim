...
void sr200pc20_set_preview(void)
{
	CAM_DEBUG("cam_mode = %d", sr200pc20_control.cam_mode);

	if (sr200pc20_control.cam_mode == MOVIE_MODE) {
		CAM_DEBUG("Camcorder_Mode_ON");
		SR200PC20_WRT_LIST(sr200pc20_Init_Reg);
		SR200PC20_WRT_LIST(sr200pc20_20fps);
	}
    else {
		if (sr200pc20_control.vtcall_mode == 1) {
			CAM_DEBUG("set VT mode");
			SR200PC20_WRT_LIST(sr200pc20_VT_Init_Reg);
        }
        else {
...
void sr200pc20_set_capture(void)
{
	CAM_DEBUG("");
...
static long sr200pc20_set_sensor_mode(int mode)
{
	CAM_DEBUG("%d", mode);

	switch (mode) {
	case SENSOR_PREVIEW_MODE:
		sr200pc20_set_preview();
		break;
	case SENSOR_SNAPSHOT_MODE:
	case SENSOR_RAW_SNAPSHOT_MODE:
		sr200pc20_set_capture();
		break;
	default:
		return 0;
	}
