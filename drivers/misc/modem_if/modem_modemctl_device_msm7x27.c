...
extern int esc_6270_force_dump;
#endif
...
static int msm7x27_force_crash_exit(struct modem_ctl *mc)
{
	...
	esc_6270_force_dump = 1;
	
	//pr_info("[MIF] <%s> Called!!\n", __func__);
	/* Do not use this func in msm8x55 */

	phone_reset = gpio_get_value(GPIO_ESC_RESET_N);

	pr_info("[MIF] <%s> Called!!%d, %d\n", __func__, phone_reset);

	
	//set phone_active low for getting force dump
	pr_info("[MIF] <%s> Called!! Set GPIO_ESC_RESET_N LOW\n", __func__);

	gpio_tlmm_config(GPIO_CFG(GPIO_ESC_RESET_N,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_16MA), GPIO_CFG_ENABLE);
	gpio_set_value(GPIO_ESC_RESET_N, 0);

	phone_active = gpio_get_value(GPIO_ESC_RESET_N);

	pr_info("[MIF] <%s> Called!! GPIO_ESC_RESET_N value is %d\n", __func__, phone_active);
#else
	pr_info("[MIF] <%s> Called!!\n", __func__);
	/* Do not use this func in msm8x55 */
#endif
